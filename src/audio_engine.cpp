...
metro::audio::Engine::OutStream
metro::audio::Engine::new_outstream(std::chrono::microseconds ticker_period)
{
	float best_latency_s = pick_best_latency(ticker_period);
	return metro::audio::Engine::OutStream(this, best_latency_s);
}
...
void metro::audio::Engine::eventloop()
{
	for (;;)
