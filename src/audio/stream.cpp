...
jungle::audio::Engine::Stream::Stream(struct SoundIoDevice* device)
{
	int err;
...
void jungle::audio::Engine::Stream::play_tone(jungle::audio::Tone& tone)
{
	...
	while (frames_left > 0) {
		...
		for (int frame = 0; frame < frame_count; ++frame) {
			...
			for (int channel = 0; channel < layout->channel_count; ++channel) {
				float* ptr = ( float* )(areas[channel].ptr
