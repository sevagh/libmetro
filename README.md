...
The following tables describe libjungle's features at a high level. Read the linked header files to find more details on constructors, members, etc.

#### jungle::core

| Header | Namespace | Component | Description |
| --------- | ----------- | -------- | ---- |
| [libjungle.h](./include/libjungle/libjungle.h) | `jungle::core` | `SampleRateHz` | Global sample rate for all libjungle components |
|| `jungle::core::event` | `Event` | Alias for a `void()` function pointer<br/>suggestion for supporting arguments: [lambda captures](https://en.cppreference.com/w/cpp/language/lambda) |
||| `class EventCycle` | Wraps a provided vector of Events and cycles through them<br/>Events are dispatched in the background using [thread detach](https://en.cppreference.com/w/cpp/thread/thread/detach) |
|| `jungle::core::tempo` | `void precise_sleep_us(dur_us)` | A function for microsecond-precision sleep<br>Strategy is measure elapsed time and sleep 1ns in a loop<br/>Accuracy tested [here](./test/clock_accuracy.cpp) |
||| `std::chrono::microseconds bpm_to_us(bpm)`<br/>`int us_to_bpm(us)` | Convert between bpm and period in microseconds |
||| `class Tempo` | Create a timer executor of EventCycles with us precision<br/>This runs in the background so you need to block after `start()` |
|| `jungle::core::audio` | `class Engine` | Wraps Soundio structs for low level audio functionality |
||| `class Engine::OutStream` | A nested class of Engine containing a Soundio struct for an output stream<br/>Note: only an engine can create OutStreams |

#### jungle::synthesis

| Header | Namespace | Component | Description |
| --------- | ----------- | -------- | ---- |
| [libjungle_synthesis.h](./include/libjungle/libjungle_synthesis.h) | `jungle::synthesis` | `class Timbre` | An abstract base class for stk-based sound generators<br/>Generated stk samples are stored in a `std::vector<float>` |
||| `class Pulse` | Implements Timbre, creates a sine tone<br/>Wraps [stk/SineWave.h](https://github.com/thestk/stk/blob/master/include/SineWave.h) |
||| `class Drum` | Implements Timbre, creates MIDI drum sounds<br/>Wraps [stk/Drummer.h](https://github.com/thestk/stk/blob/master/include/Drummer.h) |
|| `jungle::synthesis::metronome` | `map<string, EventCycle> time_signature_mappings | A map of implemented metronomes, key is time signature<br/>E.g. the common time metronome has key "4/4" |
||| `EventCycle metronome_common_time` | Creates a 4/4 metronome event cycle<br/>Intended to be registered on a Tempo instance |
...
Note that the creation of a "perfect metronome" is a non-goal of libjungle. Users should feel empowered to create any sort of metronome they need on the fly, with relatively little lines of code.
